...
-   **Built on a World-Class Foundation:** Plug-N-Meet is powered by **LiveKit** as its media server and **NATS** for real-time messaging (chat, whiteboard sync, etc.). These are extremely fast, scalable, and modern components that allow Plug-N-Meet to handle a massive user load with fewer server resources. See measured numbers in our [Benchmarks](/docs/benchmark): 300–500 users in one room ran clean on a single 4-vCore server.
