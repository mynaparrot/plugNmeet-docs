...
description: Documentación del punto final de la API para obtener una lista de artefactos de la reunión, como resúmenes y transcripciones generados por IA.
...
| ---------- | ---------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| room_ids   | array de cadenas | No       | Un array de uno o más `room_id`s para filtrar los resultados.                                                                                                               |
| room_sid   | string           | No       | Filtrar artefactos de una sesión de sala específica proporcionando su `sid`.                                                                                                   |
| type       | cadena o número | No       | Filtrar por un tipo de artefacto específico. Puede usar el nombre de la cadena (p. ej., `MEETING_ANALYTICS`) o su valor entero correspondiente. Vea los Tipos de Artefactos a continuación. |
...
| order_by   | string           | No       | El orden de clasificación para los resultados. Puede ser `ASC` o `DESC`. Predeterminado: `DESC`.                                                                                                |
...
| artifacts_list  | array de ArtifactInfo | La lista de objetos de artefacto. Consulte la página **[Obtener Información del Artefacto](./artifact-info)** para la estructura detallada del objeto `ArtifactInfo`. |

## Tipos de Artefactos

El campo `type` identifica el tipo de artefacto. Puede usar el nombre de la cadena (p. ej., `MEETING_ANALYTICS`) o su valor entero correspondiente al filtrar.
